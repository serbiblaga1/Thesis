...
class AerialRobotWithObstacles(BaseTask):
    ...
    def __init__(self, cfg: AerialRobotWithObstaclesCfg, sim_params, physics_engine, sim_device, headless):
        ...
        if self.cfg.env.enable_onboard_cameras:
            self.full_camera_array = torch.zeros((self.num_envs, 8, 8), device=self.device)
    ...
    def pre_physics_step(self, _actions):
        ...
        # Output from controller (thrusts and torques)
        # output_thrusts_mass_normalized, output_torques_inertia_normalized = self.controller(self.root_states, self.action_input)

        # # Assuming constants for drag coefficients bx, by
        # bx, by = 4.2, 1.8 

        # # Extract velocities
        # vx = self.root_linvels[:, 0]
        # vy = self.root_linvels[:, 1]

        # # Calculate forces including drag
        # F_drag_x = -bx * vx
        # F_drag_y = -by * vy
        # F_drag_z = 0  # Since v_z is assumed to be 0, F_drag_z is 0

        # # Calculate total thrust force in the z direction
        # F_thrust_z = self.robot_mass * (-self.sim_params.gravity.z) * output_thrusts_mass_normalized

        # # Summing up all forces
        # self.forces[:, 0, 0] = F_drag_x
        # self.forces[:, 0, 1] = F_drag_y
        # self.forces[:, 0, 2] = F_thrust_z
