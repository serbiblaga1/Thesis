...
class AssetManager:
    def __init__(self, config, device):
        ...
        self.asset_type_to_dict_map = {
            ...
            "tomatoes": self.cfg.tomato_asset_params,
            ...
            "inner_wall": self.cfg.inner_wall,
            "inner_small_wall": self.cfg.inner_small_wall}
    ...
    def _add_asset_2_tensor(self, asset_class):
        ...
        num_assets = asset_class.num_assets
        num_positions = len(asset_class.specified_positions)
        
        # Define the asset tensors together for the number of assets of the same class being loaded
        asset_tensor = torch.zeros((num_assets, 6), dtype=torch.float, device=self.device)
        
        min_state_tensor = torch.tensor(
            asset_class.min_position_ratio + asset_class.min_euler_angles, dtype=torch.float, device=self.device
        ).expand(num_assets, -1)
        max_state_tensor = torch.tensor(
            asset_class.max_position_ratio + asset_class.max_euler_angles, dtype=torch.float, device=self.device
        ).expand(num_assets, -1)
        
        # Create tensor for specified positions and orientations
        specified_positions_tensor = torch.tensor(
            asset_class.specified_positions, dtype=torch.float, device=self.device
        )
        specified_euler_angles_tensor = torch.tensor(
            asset_class.specified_euler_angles, dtype=torch.float, device=self.device
        )
        
        # Ensure the sizes match for each tensor
        assert specified_positions_tensor.shape[0] == num_assets
        assert specified_euler_angles_tensor.shape[0] == num_assets
        
        # Combine positions and orientations into a single tensor
        specified_state_tensor = torch.cat((specified_positions_tensor, specified_euler_angles_tensor), dim=1)
        ...
        if self.asset_pose_tensor is None:
            ...
            self.asset_pose_tensor = torch.vstack((self.asset_pose_tensor, asset_tensor))
            self.asset_min_state_tensor = torch.vstack((self.asset_min_state_tensor, min_state_tensor))
            self.asset_max_state_tensor = torch.vstack((self.asset_max_state_tensor, max_state_tensor))
            self.asset_specified_state_tensor = torch.vstack((self.asset_specified_state_tensor, specified_state_tensor))
    ...
    def randomize_pose(self, num_obstacles=None, reset_envs=None):
        ...
        # Update asset positions and orientations
        pos_ratio_euler_absolute = self.asset_min_state_tensor + torch.rand_like(self.asset_min_state_tensor) * (self.asset_max_state_tensor - self.asset_min_state_tensor)
        self.asset_pose_tensor[:, :, :3] = self.env_lower_bound.unsqueeze(1) + self.env_bound_diff.unsqueeze(1) * pos_ratio_euler_absolute[:, :, :3]
        self.asset_pose_tensor[:, :, 3:6] = pos_ratio_euler_absolute[:, :, 3:6]

        # Apply specified positions if they are valid
