...
def compute_pitch_correction(pid_params, current_distance, dt, log_file='pitch_log.csv'):
    ...
    return  pitch_correction, distance_error
...
def simple_pid_takeoff_and_move(args):
    ...
    target_distance_from_wall = 0.3
    pitch_pid_params = initialize_pitch_pid(target_distance_from_wall, (0.3, 0.2, 0.2))

    yaw_target_angle = math.radians(90) 
    current_yaw_angle = 0.0  
    yaw_pid_params = initialize_yaw_pid([0.1, 0.01, 0.05])  

    target_distance_from_wall_side = 0.27  
    roll_pid_params = initialize_roll_pid(target_distance_from_wall_side, (0.002, 0.02, 0.02))
    ...
    ############################### MOVE #############################################  
        ...
        if takeoff_complete:
            ...
            roll_correction_right, _ = compute_roll_correction(roll_pid_params, right_distance, dt)
            roll_correction_left, _ = compute_roll_correction(roll_pid_params, left_distance, dt)

            if right_distance <= target_distance_from_wall_side:
                    actions[:, 1] = -roll_correction_right
            if left_distance <= target_distance_from_wall_side:
                    actions[:, 1] = roll_correction_left
            ...
            if reached_obstacle:
                ...
                if 'yaw_start_time' not in pid_params:
                    pid_params['yaw_start_time'] = pid_params['cumulative_time']  
                
                if pid_params['cumulative_time'] - pid_params['yaw_start_time'] < yaw_duration:
                    actions[:, 3] = yaw_torque 
                    actions[:, 2] = 0
                    actions[:, 1] = 0
                    #print(f"Yawing: Time passed {pid_params['cumulative_time'] - pid_params['yaw_start_time']:.2f}s")
                else:
                    actions[:, 3] = 0 
                    reached_obstacle = False
                    del pid_params['yaw_start_time']
                    print("Yaw Completed")
            #print(actions)
