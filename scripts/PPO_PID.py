...
def get_args():
    custom_parameters = [
        ...
        {"name": "--num-steps", "type":int, "default": 128, "help": "the number of steps to run in each environment per policy rollout"},
        ...
        {"name": "--gae-lambda", "type":float, "default": 0.8, "help": "the lambda for the general advantage estimation"},
...
class PIDController:
    def __init__(self, kp, ki, kd, target=0.3):
        self.kp = kp
        self.ki = ki
        self.kd = kd
        self.target = target
    ...
    def compute(self, current_value, dt=0.02):
...
# Main Training Loop
# ============================
def train_altitude_control_with_pid(env, agent, pid_controller, args):
    ...
    actions = torch.zeros((args.num_steps, args.num_envs, 4), dtype=torch.float).to(args.rl_device)
    ...
    # To take-off and reach taregt altitude:
    stabilization_steps = 0
    stabilization_threshold = 50

    for update in range(1, num_updates + 1):
        for step in range(0, args.num_steps):
            global_step += 1
            obs_tensor = next_obs[0].unsqueeze(0) if next_obs[0].dim() == 1 else next_obs[0]
            obs[step] = obs_tensor
            dones[step] = next_done

            current_altitude = obs_tensor[:, 0].cpu().numpy() 
            pid_thrust = np.array([pid_controller.compute(alt) for alt in current_altitude])
            pid_thrust_tensor = torch.tensor(pid_thrust, dtype=torch.float32, device='cuda:0').unsqueeze(-1)                       
            
            stabilization_steps += 1

            if stabilization_steps >= stabilization_threshold:
                # RL for other actions
                with torch.no_grad():
                    rl_action, logprob, _, value = agent.get_action_and_value(obs_tensor)
                    pitch_control = rl_action[:, 0]
                    yaw_control = rl_action[:, 1]
                    roll_control = 0
                    values[step] = value.flatten()
                
                total_thrust = pid_thrust_tensor
                full_action = torch.zeros((args.num_envs, 4), dtype = torch.float32, device = args.rl_device)
                full_action[:, 0] = total_thrust.squeeze(-1)
                full_action[:, 1] = roll_control
                full_action[:, 2] = pitch_control.squeeze(-1)
                full_action[:, 3] = yaw_control
            else:
                full_action = torch.zeros((args.num_envs, 4), dtype = torch.float32, device = args.rl_device)
                full_action[:, 0] = pid_thrust_tensor.squeeze(-1)
                full_action[:, 1] = 0
                full_action[:, 2] = 0
                full_action[:, 3] = 0
                

            actions[step] = full_action
            logprobs[step] = logprob if stabilization_steps >= stabilization_threshold else 0.0
            next_obs, rewards[step], next_done, info = env.step(full_action)
        ...
        if stabilization_steps >= stabilization_threshold:
            # Train only after stabilization
...
if __name__ == "__main__":
    ...
    pid_controller = PIDController(kp=0.5, ki=0.1, kd=0.1)  

    train_altitude_control_with_pid(env, agent, pid_controller, args)